...
@interface AAPLKeychainTestsViewController ()
...
- (void)updateItemAsync
{
    NSDictionary *query = @{
                            (__bridge id)kSecClass: (__bridge id)kSecClassGenericPassword,
                            (__bridge id)kSecAttrService: @"SampleService",
                            (__bridge id)kSecUseOperationPrompt: @"更新密码进行身份验证"
                            };
...
- (void)addItemAsync
{
    ...
    // 如果不用 kSecAttrAccessibleWhenUnlocked，则删除密钥无效
    ...
    // 果我们想要操作的密钥认证失败，则弹出kSecUseNoAuthenticationUI界面
